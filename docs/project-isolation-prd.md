...
When a user runs `dgk scan <path>` for a project that already exists in the registry, the system will warn and automatically perform an incremental scan (diffing against the stored `last_scanned_at` timestamp) instead of a full rescan. A `--force` flag will allow a full rescan override. If `.dgk/project.toml` does not exist at scan time, the system will auto-initialize it.
...
3. As a developer, I want `dgk scan` to auto-initialize a project on first run, so that I don't need a separate `dgk init` step.
...
10. As a developer, I want `dgk query` and `dgk impact` to only return results from the current project, so that results are relevant.
...
**Project identity**: The canonical absolute path of the project root (resolved via `Path.resolve()`). This value is stored in `.dgk/project.toml` under `[project] id`.
...
**Scan behavior for known projects**: When `dgk scan <path>` detects an existing Project node (by resolved absolute path):
...
**Auto-init**: If `dgk scan <path>` is run on a path without `.dgk/project.toml`, the system will auto-create it (equivalent to running `dgk init`) before scanning.
...
- **`scan` command** — Verify: (a) first scan creates a Project node, (b) repeat scan prints warning and runs incrementally, (c) `--force` overrides to full scan, (d) auto-init creates `.dgk/project.toml` when missing.
...
- **`init` command** — Verify that `project_id` (absolute path) is written to `.dgk/project.toml`.
